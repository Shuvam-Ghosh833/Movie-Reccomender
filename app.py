...
movies=pd.DataFrame(movies_dict) # converting dictionary to data frame



def recommend(m):
    movie_index = movies[movies['title'] == m].index[0]  # finding the index of the given 'm'

    distance = similarity[movie_index]  # finding the distance from movie 'm' from all the other movies
